...
import { UpdateEmail, UpdatePass } from "../../../../auth/core/requests";
...
const SignInMethod: React.FC = () => {
  ...
  const [emailUpdateData, setEmailUpdateData] = useState<IUpdateEmail>({
    newEmail: currentUser?.email ? currentUser?.email : "",
    confirmPassword: "",
  });
  ...
  const [UpdateEmailF] = useMutation(UpdateEmail);
  const [UpdatePassF] = useMutation(UpdatePass);
    ...
    onSubmit: (values) => {
      ...
      UpdateEmailF({
        ...
        onCompleted: ({ UpdateEmail }) => {
          ...
          setLoading1(false);
          UpdateEmail && setShowEmailForm(false);
          if (!UpdateEmail) {
  ...
  const formik2 = useFormik<IUpdatePassword>({
    ...
    onSubmit: (values) => {
      ...
      UpdatePassF({
        ...
        onCompleted: ({ UpdatePass }) => {
          setLoading2(false);
          UpdatePass && setPasswordForm(false);
          if (!UpdatePass) {
            formik2.errors.currentPassword = "Invalid Password";
          }
        },
      });
    },
